...
const MANAGED_EXCHANGE_MARKER: &str = ".souffle-rs-managed";
...
pub(super) fn prepare_exchange_dir(path: &Path) -> Result<(), SouffleError> {
    if path.exists() {
        if !path.is_dir() {
            return Err(SouffleError::FileIo {
                operation: "prepare directory".to_owned(),
                path: path.display().to_string(),
                message: "path exists but is not a directory".to_owned(),
            });
        }

        if path.join(MANAGED_EXCHANGE_MARKER).exists() {
            fs::remove_dir_all(path).map_err(|source| SouffleError::FileIo {
                operation: "remove directory".to_owned(),
                path: path.display().to_string(),
                message: source.to_string(),
            })?;
        } else if directory_has_entries(path)? {
            return Err(SouffleError::FileIo {
                operation: "prepare directory".to_owned(),
                path: path.display().to_string(),
                message: "refusing to remove unmanaged process exchange directory".to_owned(),
            });
        }
    }
    ...
    fs::write(
        path.join(MANAGED_EXCHANGE_MARKER),
        b"generated by souffle-rs process backend\n",
    )
    .map_err(|source| SouffleError::FileIo {
        operation: "write".to_owned(),
        path: path.join(MANAGED_EXCHANGE_MARKER).display().to_string(),
        message: source.to_string(),
    })
}

fn directory_has_entries(path: &Path) -> Result<bool, SouffleError> {
    let mut entries = fs::read_dir(path).map_err(|source| SouffleError::FileIo {
        ...
        TypeRef::Union { name, variants } => {
            ...
            for variant in variants {
                match parse_output_value(relation, column, variant, field, path, definitions) {
                    Ok(value) => {
                        return Ok(Value::typed(name.clone(), value));
                    }
